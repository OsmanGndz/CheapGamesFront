          ...
          >
            {filteredData.map((item, index) => (
              <div
                ...
                className="relative flex flex-col flex-shrink-0 w-[15.66%] rounded-b-lg group overflow-hidden cursor-pointer"
              >
                <img
                  src={item.imageUrl}
                  alt={item.title}
                ...
                />
                <div className="flex flex-col text-[12px] items-center pt-2 gap-1 w-full h-20 bottom-0 bg-slate-800 rounded-b-lg z-10">
                  {item.title}
                  <div className="flex justify-center w-full h-full font-bold">
                    <span className="text-sm text-white">{item.price.toFixed(2)} TL</span>
